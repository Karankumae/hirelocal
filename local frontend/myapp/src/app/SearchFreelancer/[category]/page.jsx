...
export default function Page() {
  const pathname = usePathname(); // /freelancers/webdeveloper
  const categorySlug = pathname.split('/').pop(); // webdeveloper
  ...
  useEffect(() => {
    const fetchFreelancers = async () => {
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/freelancers/${categorySlug}`);
  ...
  return (
    ...
    <div className="min-h-screen  ">
      <h1 className="text-4xl font-bold text-center  mb-12">
        {categorySlug?.replace(/-/g, ' ')} Freelancers
      </h1>
        ...
        {freelancers.map((freelancer) => (
          ...
          >
            ...
            <div className="p-5">
              ...
              <p className="text-blue-900 text-[15px] mb-2 flex items-center gap-2">
              ...
              <p className="text-blue-900 text-[15px] flex items-center gap-2">
